...
Reviewer's: Dispersion of different bitwidths may significantly reduce speedup effects.
...
Memory bandwidth savings >> Kernel switching cost
...
Despite 3 additional kernel switches, QAdapt is 1.7× faster than uniform quantization because:
...
This is a valid theoretical concern, but empirically negligible (0.22ms = 1.2%) due to parameter grouping.
...
| Benefit | -24% | -4.3 MB | -5.4 ms | +0.22 ms | -13.2 ms |
 ...
 Memory bandwidth savings (5.4ms) >> Dispersion overhead (0.22ms)
...
Question: *"In line 161, does it mean that for every hyperedge e, QADAPT builds a learnable projection?"*
 ...
 No, we do NOT learn separate projections for each hyperedge. That would require O(|E|·d²) parameters, which is infeasible (e.g., DBLP: 22,363 edges × 1,425² = 45.4 billion parameters).
